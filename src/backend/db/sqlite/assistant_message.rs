...
pub fn row_to_assistant_message_part(row: &Row) -> Result<AssistantMessagePart, rusqlite::Error> {
    Ok(AssistantMessagePart {
        ...
        tool_state_raw: row.get(33)?,
        tool_state_time_start: row.get(34)?,
        tool_state_time_end: row.get(35)?,
        tool_state_time_compacted: row.get(36)?,
        tool_attachments_json: row.get(37)?,
        finish_reason: row.get(38)?,
        cost: row.get(39)?,
        token_total: row.get(40)?,
        token_input: row.get(41)?,
        token_output: row.get(42)?,
        token_reasoning: row.get(43)?,
        token_cache_read: row.get(44)?,
        token_cache_write: row.get(45)?,
        snapshot_hash: row.get(46)?,
        patch_hash: row.get(47)?,
        patch_files_json: row.get(48)?,
        retry_attempt: row.get(49)?,
        retry_error_json: row.get(50)?,
        retry_created_at: row.get(51)?,
        compaction_auto: row.get(52)?,
        delta_field: row.get(53)?,
        delta_text: row.get(54)?,
        part_time_start: row.get(55)?,
        part_time_end: row.get(56)?,
        part_metadata_json: row.get(57)?,
        text_synthetic: row.get(58)?,
        text_ignored: row.get(59)?,
        step_snapshot_hash: row.get(60)?,
        created_at: row.get(61)?,
        updated_at: row.get(62)?,
    })
}
        ...
        .execute(
            "UPDATE assistant_message_part
             SET
                ...
                tool_state_raw = ?34,
                tool_state_time_start = ?35,
                tool_state_time_end = ?36,
                tool_state_time_compacted = ?37,
                tool_attachments_json = ?38,
                finish_reason = ?39,
                cost = ?40,
                token_total = ?41,
                token_input = ?42,
                ...
                part_time_start = ?56,
                part_time_end = ?57,
                part_metadata_json = ?58,
                text_synthetic = ?59,
                text_ignored = ?60,
                step_snapshot_hash = ?61,
                updated_at = ?62
             WHERE id = ?1",
            params![
                &part.id,
